...
1. Now, take 15 minutes to get a big picture of the code in this application and its features.
    ...
    * Answer - Classes are BoardButton, Computer, Main, Player, PlayerFactory, Sentient, And TicTacToe
    * Some objects I noticed are the UI components, Players and the PlayerFactory
    * There is encapsualtion in The Player Factory, There is abstraction in the abstract Player Class, There is inheritance in most of the classes, And there is polymorphism in the factor.
1. Don’t be stuck for too long, if you don't understand a section jot it down below and move on.
    ...
    * Answer - The only things i saw that were unfamiliar are In the computer class Most of the Node stuff like Node arrays Also when making a combobox not sure if the <> next to it means generic? I also didnt understand in this code: BoardButton square = (BoardButton)board[r][c]; what does the [r][c] do?
